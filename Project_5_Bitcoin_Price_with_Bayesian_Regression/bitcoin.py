...
def get_dist(wt,X,Xi,i,lastIndex,Xi_prices):
    N = pd.Series.sum((X[:lastIndex]-np.mean(pd.Series(X[:lastIndex])))*(Xi.iloc[i][Xi_prices]-np.mean(Xi.iloc[i][Xi_prices])))
    D = (X.size - 1)*np.std(pd.Series(X[:lastIndex]))*np.std(Xi.iloc[i][Xi_prices])
    distance = wt*N/D
    return distance
...
def computeDelta(wt, X, Xi):
    ...
    # YOUR CODE GOES HERE
    n, d = 0, 0
    lastIndex = X.size - 1 #ignoring last index
    size = len(Xi.index) #size of Xi
    
    for i in range(0,size):
        Xi_prices = Xi.columns[:len(Xi.columns)-1]
        distance = get_dist(wt,X,Xi,i,lastIndex,Xi_prices)
        n = n + Xi.iloc[i][len(Xi.columns)-1] * math.exp(distance)
        d = d + math.exp(distance)
