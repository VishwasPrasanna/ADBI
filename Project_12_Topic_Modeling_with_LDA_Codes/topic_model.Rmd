...
```{r}
word_probabilities <- apply(exp(LDA_model@beta), 2, max)
sort_ascending <- sort(word_probabilities, decreasing=T, index.return=T)
vals <- sort_ascending$x[1:100]
words <- LDA_model@terms[sort_ascending$ix[1:100]]
# YOUR CODE HERE
```

Now, plot the top-100 values.
```{r}
plot(1:100, vals, "b", xlab="Word", ylab="Max Probability",col="dark red")
# YOUR CODE HERE
```
...
k = 20
while(k<=200){
  LDA_model <- LDA(train_data, control=list(iter=20, seed=SEED), k=k, method="Gibbs")
  k <- k+20
  LDA_10_Models = c(LDA_10_Models,LDA_model)
}

perplexity_model <- function(x) perplexity(x, test_data)
perplex <- sapply(LDA_10_Models, perplexity_model)
plot(perplex, type="b", col="dark red",axes = F, xlab="Number of topics", ylab="Perplexity", main="Perplexity growth")
...
```{r}
yelp = read.csv2("yelp.txt", header=FALSE, quote="", sep="|")
...
k <- 10
while(k<=100){
  LDA_model <- LDA(train_data, control=list(iter=100, seed=SEED), k=k, method="Gibbs")
  k<-k+10
  LDA_10_Models = c(LDA_10_Models,LDA_model)
}

NPMI_values <- function(LDA_model) compute_c(LDA_model@beta, test_data, top_N = 15, "NPMI")
NPMI = sapply(LDA_10_Models,NPMI_values)
NPMI_results = data.frame(seq(10,100,10), NPMI)
plot(NPMI_results, type="b", col="dark red", xlab="Number of topics", ylab="NPMI", main="Extrinsic topic coherence")
...
```{r}
Compute_c_results <- function(LDA_model) compute_c(LDA_model@beta,yelp_matrix[-(1:1000),],top_N = 15, "LCP")
LCP = sapply(LDA_10_Models, Compute_c_results)
LCP_results = data.frame(seq(10,100,10),LCP)
plot(LCP_results, type="b", col="dark red",xlab="Number of topics", ylab="Log Likelihood", main="Intrinsic Topic Coherence")
# YOUR CODE HERE
```
...
k <- 10
while(k<=100){
  LDA_model <- LDA(yelp_10000, control=list(iter=10, seed=SEED), k=k, method="Gibbs")
  k<-k+10
  LDA_10_models <- c(LDA_10_models, LDA_model)
}
# YOUR CODE HERE
...
```{r}
clusters <- list()
cluster_val <- function(x) sort(x, decreasing=T, index.return = T)$ix[1]
for(LDA_model in LDA_10_models){
  cluster <- list(apply(LDA_model@gamma, 1, cluster_val))
...
purity <- function(cluster, labels){
  ...
  for (i in unique(cluster)){
    document_number <- which(cluster==i)
    document_labels <- labels[document_number]
    label_counts <- sapply(unique(document_labels), function(x) length(which(document_labels==x)))
...
plot(purity, type="b", col="dark red", xlab="Number of Topics", ylab="Purity", main="Document Clustering Purities") 
# YOUR CODE HERE
```
