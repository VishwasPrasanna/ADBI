...
def records_to_graph():
    ...
    nodedict = {}
    # YOUR CODE HERE
    def create_new_node(node_names, node_types):
      for n,name in enumerate(node_names):
        nnode = Node(len(nodelist)+n, name, node_types)
        nodelist.append(nnode)
        nodedict[nnode.name] = nnode
        
    nodety = ["user","movie","director","actor","genre"]
    for val in nodety:
      if val=="genre":
        create_new_node(genres,val)
      elif val=="director":
        create_new_node(directors,val)
      elif val=="actor":
        create_new_node(actors,val)
      elif val=="user":
        create_new_node(ratings.keys(),val)
      else:
        create_new_node(movies.keys(),val)

    
    for i in range(1,6):
        mratings = []
        for mid in movies:
                mratings.append(mid+str(i))
        create_new_node(mratings,"rating")

    # Add edges between users and movie-rating nodes
    # YOUR CODE HERE
    def create_neighbors(x,mov):
        n = nodedict[x]
        n.neighbors.append(mov)
        mov.neighbors.append(n)

    for user in ratings:
        for (movie,rating) in ratings[user].items():
            mr_node = nodedict[movie+rating]
            create_neighbors(user,mr_node)

    for movie_id in movies:
        m = movies[movie_id]
        nMovie = nodedict[movie_id]

        if m.director != None:
            create_neighbors(m.director,nMovie)

        for a in m.actors:
            create_neighbors(a,nMovie)
            
        for g in m.genres:
            create_neighbors(g,nMovie)


        for mr in [movie_id+str(i) for i in range(1,6)]:
            create_neighbors(mr,nMovie)
