...
def load_data(path_to_dir):
    ...
    test_pos = []
    test_neg = []
    with open(path_to_dir+"train-pos.txt", "r", encoding="utf-8") as f:
        for i,line in enumerate(f):
            words = [w.lower() for w in line.strip().split() if len(w)>=3]
            if (len(words) == 0): continue
            train_pos.append(words)
    with open(path_to_dir+"train-neg.txt", "r", encoding="utf-8") as f:
        for line in f:
            words = [w.lower() for w in line.strip().split() if len(w)>=3]
            if (len(words) == 0): continue
            train_neg.append(words)
    with open(path_to_dir+"test-pos.txt", "r",encoding="utf-8") as f:
        for line in f:
            words = [w.lower() for w in line.strip().split() if len(w)>=3]
            if (len(words) == 0): continue
            test_pos.append(words)
    with open(path_to_dir+"test-neg.txt", "r",encoding="utf-8") as f:
        for line in f:
            words = [w.lower() for w in line.strip().split() if len(w)>=3]
            if (len(words) == 0): continue
...
def feature_vecs_NLP(train_pos, train_neg, test_pos, test_neg):
    ...
    #now finding feature that meet second condition
    potential_features = []
    for word in neg_count:
        check = (float(neg_count[word])/len(neg_count))*100
        if check >= 1: 
            potential_features.append(word)
    for word in pos_count:
        check = (float(pos_count[word])/len(pos_count))*100
        if check >= 1: 
            potential_features.append(word)

    #features that meet 1st condition
    new_potential_features = []
    for word in potential_features:
        if word not in stopwords:
            new_potential_features.append(word)

    #features the meet 1st, 2nd and 3rd condition
    features = []
    for word in new_potential_features:
        check1 = pos_count[word]>=2*neg_count[word]
        check2 = neg_count[word]>=2*pos_count[word]
        if check1 or check2:
...
def evaluate_model(model, test_pos_vec, test_neg_vec, print_confusion=False):
    ...
    # YOUR CODE HERE
    positive = list(model.predict(test_pos_vec))
    negative = list(model.predict(test_neg_vec))
    tp = positive.count("pos")
    fp = negative.count("pos")
    tn = negative.count("neg")
    fn = positive.count("neg")
