...
def simple(g):
    ...
    # YOUR CODE HERE
    flipped_Edges = g.edges.selectExpr('dst as src','src as dst')
    ...
    # YOUR CODE HERE
    new_Edges = flipped_Edges.unionAll(g.edges).distinct().filter('src != dst')
    return GraphFrame(g.vertices, new_Edges)
...
def degreedist(g):
    ...
    # YOUR CODE HERE
    DF = g.inDegrees.selectExpr('id as id','inDegree as degree').groupBy('degree').count()
    return DF
